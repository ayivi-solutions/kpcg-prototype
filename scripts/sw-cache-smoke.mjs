...
const expectedCache='kpcg-kpcg-v18.0-continuous-20260914';
...
try{
  ...
  const shell=await page.evaluate(async expected=>{const cache=await caches.open(expected);const keys=(await cache.keys()).map(r=>new URL(r.url).pathname);const required=['/','/legacy.html','/manifest.webmanifest','/ip-guard.css','/ip-guard.js','/favicon.ico','/assets/kpcg-logo.webp'];return {keys,missing:required.filter(path=>!keys.includes(path))};},expectedCache);if(shell.missing.length)throw new Error(`v18 continuous shell assets missing from cache: ${shell.missing.join(', ')}`);
  summary={release:expectedRelease,baseURL,registration,expectedCache,cacheKeys,staleCaches,expectedCachePresent,shell,passed:true};await context.close();
}catch(error){summary={release:expectedRelease,baseURL,expectedCache,passed:false,error:error.stack||String(error)};process.exitCode=1;}finally{await browser.close();}
fs.writeFileSync(outputPath,JSON.stringify(summary,null,2)+'\n');console.log(`${summary.passed?'PASS':'FAIL'} KPCG ${expectedRelease} service-worker cache retirement: ${path.relative(root,outputPath)}`);
...
try{
 ...
 if(!source.includes('/ip-guard.js?v=17.1')||!source.includes('/ip-guard.css?v=17.1')||!source.includes('Proprietary evaluation prototype')) throw new Error('Root IP-guard wiring missing.');
 ...
 await page.waitForFunction(()=>window.__AYIVI_IP_GUARD__?.authorized===true&&document.querySelector('.ayivi-ip-watermark'),null,{timeout:15000});
 ...
 summary={mode:productionMode?'production':'preview',baseURL,passed:true,sourceMaps:0,headers:{xFrameOptions:h['x-frame-options'],xContentTypeOptions:h['x-content-type-options'],crossOriginResourcePolicy:h['cross-origin-resource-policy'],contentSecurityPolicy:csp,permissionsPolicy:h['permissions-policy']},controls,pageErrors,generatedAt:new Date().toISOString()}; await context.close();
}catch(error){summary={mode:productionMode?'production':'preview',baseURL,passed:false,error:error.stack||String(error),generatedAt:new Date().toISOString()};process.exitCode=1;}finally{await browser.close();}
fs.writeFileSync(outputPath,JSON.stringify(summary,null,2)+'\n'); console.log(`${summary.passed?'PASS':'FAIL'} KPCG IP hardening: ${path.relative(root,outputPath)}`);
